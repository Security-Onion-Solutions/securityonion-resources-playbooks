    ...
    def _expand_playbook_templates(self, playbook_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, question in enumerate(final_playbook['questions']):
            if 'query' in question:
                ...
                # Check if range is missing and add default based on template
                if 'range' not in question and isinstance(query, dict) and 'template' in query:
                    template_name = query['template']
                    if template_name in default_ranges:
                        ...
                        ordered_question['range'] = default_ranges[template_name]
                        ...
                        range_added_count += 1
                        print(f"📅 Added default range {default_ranges[template_name]} for {template_name} template")
